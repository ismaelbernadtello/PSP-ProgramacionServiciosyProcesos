...
def proceso2(diccionario_pids):
    ...
    if psutil.pid_exists(diccionario_pids['pid1']):
        print("Prioridad del proceso 1 antes de cambiar: ", psutil.Process(diccionario_pids['pid1']).nice())
        psutil.Process(diccionario_pids['pid1']).nice(10)
        print("Prioridad del proceso 1 después de cambiar: ", psutil.Process(diccionario_pids['pid1']).nice())
    else:
...
if __name__ == "__main__":
    ...
    p1 = multiprocessing.Process(target=proceso1, args=(diccionario_pids,PROCESO_PADRE_PID))
