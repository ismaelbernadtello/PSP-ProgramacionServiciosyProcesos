...
def proceso1(diccionario_pids):
    ...
    os.system("nano") # Con Gedit el proceso no muere pero con nano si. USAR NANO SI ES UN EDITOR DE TEXTO
...
def proceso2(diccionario_pids):
    ...
    # Mata a cualquier proceso de gedit que haya abierto, porque no nos deja matar a ese proceso en concreto.
    # os.system("killall gedit")
    
    # Como esto no me funciona mato desde el terminal todos los procesos de gedit
    if psutil.pid_exists(diccionario_pids['pid1']):
        print("Proceso 1 va a morir")
        print("Proceso 1 existe con id: ", diccionario_pids['pid1'])
        psutil.Process(diccionario_pids['pid1']).terminate() # Mata el proceso pero pregunta antes de matarlo
        #psutil.Process(diccionario_pids['pid1']).kill()   Mata directamente el proceso
